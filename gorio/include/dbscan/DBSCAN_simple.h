...
public:
    ...
    void extract(std::vector<pcl::PointIndices>& cluster_indices) {
        ...
        for (int i = 0; i < input_cloud_->points.size(); i++) {
            ...
            double norm = std::sqrt(input_cloud_->points[i].x * input_cloud_->points[i].x + 
                input_cloud_->points[i].y * input_cloud_->points[i].y + 
                input_cloud_->points[i].z * input_cloud_->points[i].z);
            double tmpeps_ = std::abs(norm - 1)/50 + eps_;
            int nn_size = radiusSearch(i, tmpeps_, nn_indices, nn_distances);
            if (nn_size < minPts_) {
                is_noise[i] = true;
                continue;
            }
            ...
            while (sq_idx < seed_queue.size()) {
                ...
                double tmp_eps_ = (std::sqrt(input_cloud_->points[cloud_index].x * input_cloud_->points[cloud_index].x + 
                    input_cloud_->points[cloud_index].y * input_cloud_->points[cloud_index].y + 
                    input_cloud_->points[cloud_index].z * input_cloud_->points[cloud_index].z) - 1)/100 + eps_;
                nn_size = radiusSearch(cloud_index, tmp_eps_, nn_indices, nn_distances);
                if (nn_size >= minPts_) {
                    for (int j = 0; j < nn_size; j++) {
