                                         ...
                                         sensor_msgs::PointCloud2& outlier_radar_msg)
{
  ...
  // std::cout << "time: " << radar_scan_msg.header.stamp << ", v_r: " << v_r.transpose() << std::endl;
  return success;
}
                                                  ...
                                                  std::vector<uint>& outlier_idx_best)
{
  ...
  {
    const bool weighted = config_.elevation_sigma_deg > 0.0f && v_r.norm() > 0.1;
    Matrix radar_data_inlier(inlier_idx_best.size(), weighted ? 5 : 4);
    for (uint i = 0; i < inlier_idx_best.size(); ++i)
    {
      radar_data_inlier.row(i).head<4>() = radar_data.row(inlier_idx_best.at(i)).head<4>();
      if (weighted)
      {
        const Real dx = radar_data(inlier_idx_best.at(i), 0);
        const Real dy = radar_data(inlier_idx_best.at(i), 1);
        const Real dz = radar_data(inlier_idx_best.at(i), 2);
        const Real rho = std::sqrt(dx * dx + dy * dy);
        // d is already a unit vector, so sin(el) = dz and cos(el) = rho
        const Vector3 e_el(-dz * (rho > 1e-9 ? dx / rho : 0.0),
                           -dz * (rho > 1e-9 ? dy / rho : 0.0),
                           rho);
        const Real s_el = angles::from_degrees(config_.elevation_sigma_deg);
        const Real s_res = s_el * std::fabs(e_el.dot(v_r));
        const Real var = config_.doppler_sigma * config_.doppler_sigma + s_res * s_res;
        radar_data_inlier(i, 4) = (var > 1e-12) ? (config_.doppler_sigma * config_.doppler_sigma / var) : 1.0;
      }
    }
                                            ...
                                            bool estimate_sigma)
{
  ...
  // A fifth column means per-row weights: scaling each row of H and y by the
  // square root of its weight turns the ordinary fit into the weighted one,
  // and leaves every expression below unchanged.
  if (radar_data.cols() >= 5)
  {
    const Vector w = radar_data.col(4).array().max(0.0).sqrt();
    for (int i = 0; i < 3; ++i) H.col(i) = H.col(i).array() * w.array();
    y = y.array() * w.array();
  }
